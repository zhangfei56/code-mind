...
相关文档：[architecture.md](./architecture.md) · [implementation.md](./implementation.md) · [cli-guide.md](./cli-guide.md)
...
与 [architecture.md §3.9](./architecture.md#39-security) 一致：**按能力风险审批，不按「工具名遗漏」默认 Unknown。**
...
以下 demo 描述 **期望行为**；CLI 命令见 [cli-guide.md](./cli-guide.md)。
...
```text
Step 1  主：list_dir
Step 2  主：read docs/implementation.md
...
```text
subagent-policy.md（本 spec）> implementation.md（现状）> 模型自行推断
```

当实现与 spec 不一致时，以 **本 spec 为产品目标**，在 implementation.md 或 PR 中标注差距。

---

## 相关文档

- [implementation.md §5 模块归属](./implementation.md#5-模块归属避免重复实现) — SubagentManager 唯一归属
- [architecture.md §3.2](./architecture.md#32-agent-orchestration-编排层) — 编排层职责