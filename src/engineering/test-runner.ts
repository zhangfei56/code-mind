...
import type { TestFailureSummary, TestProfile, TestResult } from "../shared/types.js";
...
export class DefaultTestRunner {
  async detect(projectPath: string): Promise<TestProfile> {
    if (existsSync(join(projectPath, "package.json"))) {
      ...
      return {
        ...
        framework: "node",
        commands: {
          ...(pkg.scripts?.test ? { test: "npm test" } : {}),
          ...(pkg.scripts?.lint ? { lint: "npm run lint" } : {}),
          ...(pkg.scripts?.build ? { build: "npm run build" } : {}),
          ...(existsSync(join(projectPath, "tsconfig.json"))
            ? { typecheck: `${resolveTypeScriptCommand()} --noEmit` }
            : {}),
        },
      };
    }
  ...
  async run(projectPath: string, command: string): Promise<TestResult> {
