...
function getPatchPathDecision(
  ...
  if (mode === "auto_edit") {
    if (isAskWritePath(path)) {
      return { type: "ask", reason: `Patch target "${path}" requires approval in auto_edit mode.` };
    }
    if (isDefaultAutoEditablePath(path)) {
      return { type: "allow" };
    }
    return { type: "ask", reason: `Patch target "${path}" is outside the default auto_edit allowlist.` };
  }

  if (mode === "full_auto" || mode === "sandbox_auto") {
    if (isAskWritePath(path)) {
      return { type: "ask", reason: `Patch target "${path}" requires approval in ${mode} mode.` };
    }
    return { type: "allow" };
  }
        ...
        switch (getShellPermission(command, mode)) {
          ...
          case "deny":
            return {
              ...
              reason:
                mode === "read_only"
                  ? "Command is blocked in read_only mode."
