...
class McpClient {
  ...
  constructor(config: McpServerConfig, cwd: string) {
    ...
    logProcess("execution.mcp", "debug", "Spawned MCP client.", {
  ...
  async send(request: McpProtocolRequest): Promise<McpProtocolResponse> {
    logProcess("execution.mcp", "debug", "Sending MCP request.", request);
    ...
    while (Date.now() - startedAt < 5_000) {
      ...
      if (index >= 0) {
        ...
        logProcess("execution.mcp", "debug", "Received MCP response.", response);
        return response;
      }
